...
class actorCritic:
    ...
    def __init__(self, proprioception_input, vision, goal_vision_input, ef_vision_input, multinet, goal_list, LMA, max_step, max_trial, DOF=2):
        ...
        "noise param"
        self.noise_sigma = 4.5
        
        "learning parameters"
        self.ACT_ETA =  8.0 * 10 ** (-1)
        self.CRIT_ETA = 8.0 * 10 ** (-4)
        self.DISC_FACT = 0.99
    ...
    def train_act(self):    
        self.w_act += self.ACT_ETA * self.surp * np.outer(self.prv_state, self.prv_noise)  * self.prv_act_I * (1.- self.prv_act_I) 

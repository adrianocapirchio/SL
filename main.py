             ...
             #       if step > 0:
                    ...
                    if save_data == True:
                        ...
                        if epoch < training_epoch:
                            ...
                            if trial == 0 or trial == 2 or trial == 4 or trial == 6:
                                game.goal_angles_training_data[:,step,trial,epoch] = np.array([0.69184741,  2.32785355])
                            elif trial == 1:
                                game.goal_angles_training_data[:,step,trial,epoch] = np.array([1.3535686,   1.31873053])
                            elif trial == 3:
                                game.goal_angles_training_data[:,step,trial,epoch] = np.array([0.52349819,  1.63029882])
                            elif trial == 5:
                                game.goal_angles_training_data[:,step,trial,epoch] = np.array([ 1.063347,    1.34615867])
                            ...
                            if trial == 0 or trial == 2 or trial == 4 or trial == 6:
                                game.goal_angles_test_data[:,step,trial,epoch - training_epoch] = np.array([0.67574295,  2.3234221])
                            elif trial == 1:
                                game.goal_angles_test_data[:,step,trial,epoch - training_epoch] = np.array([1.39358439,  1.19088829])
                            elif trial == 3:
                                game.goal_angles_test_data[:,step,trial,epoch - training_epoch] = np.array([0.6179991,   1.56817767])
                            elif trial == 5:
                                game.goal_angles_test_data[:,step,trial,epoch - training_epoch] = np.array([0.98103487,  1.55417135])
                    ...
                    if cerebellum == True:                        
                        ...
                        if ataxia == False:                        
                            ...
                            cb.damage_I = ((1.0 - cb.dT / cb.tau1) * cb.damage_I + (cb.dT / cb.tau1) * (cb.curr_I )) + np.random.normal(0.0, ataxia_mag * 0.0003, 2 )  #damaged frequency
                        ...
                        if cerebellum == True:
                            if (bg.curr_rew > 0):  
                                for i in xrange(step + 1): 
                                    cb.train(bg.state_buffer[:,i], bg.act_I_buffer[:,i], bg.curr_rew)
